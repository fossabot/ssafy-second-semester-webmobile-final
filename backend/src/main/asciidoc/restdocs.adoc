...
operation::data-create-exception[snippets='http-response,response-fields']

=== DataNotFoundException
* 찾는 데이터가 없을 때 발생하는 exception +

operation::data-not-found-exception[snippets='http-response,response-fields']

=== NoAuthenticationException
* request header에 auth정보가 없는 경우 발생하는 exception +

operation::no-authentication-exception[snippets='http-response,response-fields']

=== ParameterException
* URI에 있는 Id와 Body에 있는 Id가 다를 때 발생하는 exception +

operation::parameter-exception[snippets='http-response,response-fields']

=== MethodArgumentNotValidException
* Not null 필드에 null 값이 들어있는 경우 발생하는 exception +

operation::method-argument-not-valid-exception[snippets='http-response,response-fields']
