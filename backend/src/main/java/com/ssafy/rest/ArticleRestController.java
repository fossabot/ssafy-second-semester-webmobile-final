...
@CrossOrigin
@RestController
@RequestMapping(value = "/api/bears/articles/{article_category_name}")
public class ArticleRestController {

	ArticleServiceImpl articleService;

	@Autowired
	public ArticleRestController(ArticleServiceImpl articleService) {
	...
	public ResponseEntity<List<Article>> selectAllArticles(
		...
		return list.size() != 0 ? new ResponseEntity<List<Article>>(list, HttpStatus.OK)
